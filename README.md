...
吹き出しに入力して [検索(S)]（Enter / Alt+S）で、選んだ検索エンジンで検索し、結果を作業ウィンドウ内に表示します。

- **検索エンジン**: Google / Bing / Wikipedia。iframe への埋め込みを拒否しない検索先だけを載せています（Yahoo! / YouTube / 楽天 / メルカリなどは拒否するため、ペインには出せません）。
