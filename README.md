...
- **ペイン内に結果を表示**: Bing / Wikipedia（選択肢に「（ペイン内）」と出ます）。埋め込みを拒否しない検索先だけです。
- **別タブで開く**: Google / Yahoo! / YouTube / 楽天 / メルカリ / ヤフオク / ニコニコ / X / Amazon など。これらは `X-Frame-Options` などで iframe への埋め込みを拒否するため、ブラウザの制限で、ペインには出せません。
