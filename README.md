...
吹き出しに入力して [検索(S)]（Enter / Alt+S）で、選んだ検索エンジンで検索します。

- **ペイン内に結果を表示**: Google / Bing / Wikipedia（選択肢に「（ペイン内）」と出ます）。埋め込みを拒否しない検索先だけです。Google は非公式のパラメーター（`igu=1`）で表示できるだけなので、いつ使えなくなるか、確認画面（reCAPTCHA）が出るかは、環境しだいです。
- **別タブで開く**: Yahoo! / YouTube / 楽天 / メルカリ / ヤフオク / ニコニコ / X / Amazon など。これらは `X-Frame-Options` などで iframe への埋め込みを拒否するため、ブラウザの制限で、ペインには出せません。
